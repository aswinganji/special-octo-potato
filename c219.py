...
keylist = list(object_id.keys())
valuelist = list(object_id.values())
...
def semantic_lidar_data(point_cloud_data):
    ...
    for detection in point_cloud_data:
        position = valuelist.index(detection.object_tag)
        distance = math.sqrt((detection.point.x**2) + (detection.point.y**2) + (detection.point.z**2))
        distance_name_data["distance"] = distance
        distance_name_data['name'] = keylist[position]
        print(" Name Of Surroundings - " ,  distance_name_data[name])
